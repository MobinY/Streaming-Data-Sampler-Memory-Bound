...
\begin{algorithm}[H] 
  ...
  \begin{algorithmic}[1] 
    \State $m\leftarrow \sqrt{n \log\frac{1}{\delta}}$ \Comment{We take the floor whenever a parameter is supposed to be an integer}
    \State $K\leftarrow \frac{1}{16}\log \frac{1}{\delta}$
    \State $R\leftarrow K\log(m/4K)$
    \For {$r = 0, \ldots, R$}
...
\begin{theorem}
  $\s = \Omega(\log \frac{1}{\delta}\log^2 \frac{n}{\log (1/\delta)} )$, given that $2^{10} \le \log \frac{1}{\delta} \le \frac{n}{2^{10}}$.
\end{theorem}
...
\begin{proof}
  ...
  By Lemma~\ref{lemma:bits-saving}, $\log (^n_m)-\E(\log (^n_{|B|}))\ge \frac{2}{5}R\cdot \log (\frac{n}{m}-1) \ge \frac{1}{6}R\log (\frac{n}{\log(1/\delta)})$. 
  Combining together we get $\s=\Omega(\log \frac{1}{\delta}\log^2 \frac{n}{\log (1/\delta)} )$.
\end{proof}