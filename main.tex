...
Let us consider failure probability of $\query(\sketch(\chi_A), \chi_S)$ where $S\subset A$ in terms of information leak, which is defined as the mutual information between $S$ and randomness in $\query(\sketch(\chi_A), \cdot)$. 
We show in this section that the failure probability is upper-bounded by information leak divided by $\log \frac{1}{\delta}$. 
Therefore, in order to $\query$ with good probability, it is sufficient to control information leak.
