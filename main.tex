...
On round $r$ ($r=1,\ldots, R$), $\enc$ computes $s_r\leftarrow \query(M, \mathbf{1}_{S\backslash A_{r-1}})$. 
...
$\dec$ can achieve this because of the shared random permutation $\pi$ when constructing $A_r$.
...
\begin{algorithm}[H] 
  ...
  \begin{algorithmic}[1] 
    ...
    \EndFor
    \State Let $\pi$ be a random permutation on $[n]$ \Comment{Used to generate $A_r$ and $C_r$}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H] 
  ...
  \begin{algorithmic}[1]
    ...
    \For {$r=1,\ldots,R$}
      ...
      \EndIf
      \State Remove $|A_r|-n_r$ elements from $A_r$ with smallest $\pi_a$'s among $a\in A_r$ \Comment{So that $|A_r|=n_r$}
    \EndFor
...
\begin{proof}
  ...
  After that, \enc repeats removing $a$ from $A_r$ with smallest $\pi_a$ until $|A_r|=n_r$. 
  ...
  We have $B\backslash C_r\subseteq S\backslash C_r=A_r$. Let $a^*$ denote  $a\in A_r$ that minimizes $\pi_a$. 
  We can prove that $a^*\in B\backslash C_r\subseteq A_r$ (since $a^*$ will be removed from $A_r$, it has no chance to be included in $S$ in \enc, so that $B$ contains $a^*$), and $\pi_{a^*}$ is also the smallest among $\{\pi_a|a\in B\backslash C_r\}$. 
