...
\begin{algorithm}[H] 
  ...
  \begin{algorithmic}[1] 
    ...
    \For {$r = 0, \ldots, R$}
      ...
      \newline
     \Comment{$|S_r|=n_r$, and $\forall r\ n_r-n_{r+1}\ge 2$}
    \EndFor
    \State $\pi$ is a random permutation on $[n]$      
    \newline
    \Comment{Used to generate $S_r$ and $C_r$}
  \end{algorithmic}
\end{algorithm}
...
\begin{algorithm}[H] 
  ...
  \begin{algorithmic}[1]
    ...
    \Comment{at end of round $r$, $\dec$ still needs to recover $S_r$}
    \For {$r=1,\ldots,R$}
      \State $s_r\leftarrow \query(M, \mathbf{1}_{S\backslash S_{r-1}})$       \newline
        \Comment{$s_r\mathbin{\stackrel{\rm ?}{\in}} S_{r-1}$ found in round $r$}
      \State $S_r\leftarrow S_{r-1}$
      \If {$s_r\in S_{r-1}$}       \newline
        \Comment{i.e. if $s_r$ is a valid sample}
        \State $b_r\leftarrow 1$       \newline
        \Comment{$b\in\{0,1\}^R$ indicating which rounds succeed}
