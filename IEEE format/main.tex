...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State $A\leftarrow \emptyset$ \begin{flushright}\Comment{the subset of $S$ $\dec$ recovers just from $M$} \end{flushright}
    \State $C_0 \leftarrow \emptyset$ \Comment{subset of $S$ we have built up so far}
    \For {$r=1,\ldots,R$} \begin{flushright}\Comment{each iteration tries to recover $1$ element via $M$} \end{flushright}
      \State $C_r\leftarrow C_{r-1}$
      \If{$b_r=1$} \begin{flushright}\Comment{this means Bob succeeds in round $r$} \end{flushright}
        \State $s_r\leftarrow \query(M, \mathbf{1}_{C_{r-1}})$ \begin{flushright}\Comment{Invariant: $C_r=S \backslash S_r$ ($S_r$ is defined in $\enc$)} \end{flushright}
        \State $A\leftarrow A \cup \{s_r\}$
...
\begin{equation*}
\begin{split}
H(X|Y)&\le H_2(\delta)+(1-\E(f(X,Y)))\cdot b+ \\ 
&\E(f(X,Y))\cdot (b-\log\frac{1}{\delta})\\
& = b+H_2(\delta)-\E(f(X,Y))\cdot \log\frac{1}{\delta}.
\end{split}
\end{equation*}
